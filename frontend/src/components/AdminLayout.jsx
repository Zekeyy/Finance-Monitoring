...
import { HomeIcon, ListBulletIcon, UsersIcon } from '@heroicons/react/24/outline';
function AdminLayout() {
    ...
    // Load headerText from sessionStorage or set to "Dashboard"
    ...
    useEffect(() => {
        ...
        const path = location.pathname;
        if (path.includes('/transaction-histories')) {
    ...
    useEffect(() => {
        ...
        window.addEventListener("beforeunload", resetHeader);
        
        return () => {
            window.removeEventListener("beforeunload", resetHeader);
        };
    }, []);
    ...
    return (
        <div id="defaultLayout" className="flex h-screen">
            ...
            >
                ...
                <Link to="/admin-dashboard" onClick={() => handleHeaderChange("Dashboard")}>
                    <span className="flex items-center text-white text-lg font-semibold py-2 px-4 rounded-md hover:bg-blue-500 hover:text-white transition-all duration-300 ease-in-out">
                        <HomeIcon className="h-7 w-7"style={{marginRight: '17px'}} />
                        Dashboard
                    </span>
                </Link>

                <Link to="/transaction-histories" onClick={() => handleHeaderChange("Transaction History")}>
                    <span className="flex items-center text-white text-lg font-semibold py-2 px-4 rounded-md hover:bg-blue-500 hover:text-white transition-all duration-300 ease-in-out">
                    <ListBulletIcon className="h-7 w-7" style={{marginRight: '17px'}} />
                    Transaction List
                    </span>
                </Link>

                <Link to="/user-list" onClick={() => handleHeaderChange("User Management")}>
                    <span className="flex items-center text-white text-lg font-semibold py-2 px-4 rounded-md hover:bg-blue-500 hover:text-white transition-all duration-300 ease-in-out">
                ...
                {userData && (
                    <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-amber-600/40">
                        <div className="flex items-center px-4 py-2">
                            <div className="w-10 h-10 rounded-full bg-white text-[#333333] flex items-center justify-center font-bold mr-3">
